...
class Zombie:
    ...
    def build_behavior_tree(self):
        ...
        c1 = Condition('Is boy nearby?', lambda: self.is_boy_nearby(7))
        c2 = Condition('Does zombie have more balls?', self.is_zombie_ball_more)
        c3 = Condition('Does zombie have fewer balls?', self.is_zombie_ball_less)

        a3 = Action('Move to boy', self.move_to_boy)
        a4 = Action('Move away from boy', self.move_away_from_boy)

        chase_boy = Sequence('Chase boy', c1, c2, a3)
        flee_boy = Sequence('Flee from boy', c1, c3, a4)

        chase_or_flee = Selector('Chase or Flee', chase_boy, flee_boy, wander)
